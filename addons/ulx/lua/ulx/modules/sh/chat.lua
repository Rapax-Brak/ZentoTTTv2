...
------------------------------ Gag ------------------------------
function ulx.gag( calling_ply, target_plys, should_ungag, reason )
	local players = player.GetAll()
	for i=1, #target_plys do
		local v = target_plys[ i ]
	...
	if not should_ungag then
		ulx.fancyLogAdmin( calling_ply, "#A gagged #T (#s)", target_plys, reason )
	else
...
gag:addParam{ type=ULib.cmds.StringArg, hint="reason" }
